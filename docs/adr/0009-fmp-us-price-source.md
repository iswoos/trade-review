...
**정정(프로덕션 검증 단계에서 발견):** 검색에는 `search-symbol`이 아니라 `search-name` 엔드포인트를 쓴다. `search-symbol`은 티커 접두어 매칭이라 "apple", "삼성" 같은 회사명 검색어에는 빈 배열을 반환해 사실상 검색이 죽는다 — 로컬 vitest(모킹된 fetch라 URL 경로 자체를 검증하지 않음)에서는 안 걸리고, 실제 배포 스모크 테스트에서만 드러났다. 같은 이유로, 서버리스 함수 배포도 로컬 `tsc`/`vitest`만으로는 잡히지 않는 실패가 있었다(`api/_lib/fmp` 상대 임포트에 `.js` 확장자 누락 → `type: module` 하 Vercel Node 런타임에서 `ERR_MODULE_NOT_FOUND`로 전체 엔드포인트가 죽음). 두 문제 모두 실제 `vercel --prod` 배포 후 프로덕션 URL을 curl로 찔러봐야만 드러나는 종류라, 이후 이 계열 작업은 로컬 테스트 통과만으로 끝내지 않고 프로덕션 스모크 테스트까지 반드시 거친다.
